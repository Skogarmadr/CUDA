...
#pragma omp parallel for
	...
	for (int j = 0; j < w; j++)
	    ...
	    vagueMath.colorIJ(&ptrTabPixels[s], i, j, t); // on pas l'adresses
	    }
	}
    }
...
#pragma omp parallel //pattern entrelacement
	{
	...
	while (s < WH)
	    {
	    IndiceTools::toIJ(s, w, &i, &j); // s[0,W*H[ --> i[0,H[ j[0,W[ &pour modifier l'addresse
